...
public abstract class AnvilMenuMixin extends ItemCombinerMenu {
    ...
    private int noEnchantCap$uncappedAnvil(Enchantment enchantment, Operation<Integer> original) {
        ...
        if (NoEnchantCap.getConfig().isUNCAPPED_ANVILS()){//this.player.level.getGameRules().getBoolean(NoEnchantCap.UNCAPPED_ANVILS)) {
    ...
    private boolean noEnchantCap$moreCombinations(Enchantment enchantment1, Enchantment enchantment2, Operation<Boolean> original) {
        if (NoEnchantCap.getConfig().isMIX_ANY_ENCHANTS()){//this.player.level.getGameRules().getBoolean(NoEnchantCap.MIX_ANY_ENCHANTS)) {
            return true;
        }
    ...
    private Abilities noEnchantCap$enchantAnyItem(Player player, Operation<Abilities> original) {
        if (NoEnchantCap.getConfig().isENCHANT_ANY_ITEM()){//this.player.level.getGameRules().getBoolean(NoEnchantCap.ENCHANT_ANY_ITEM)) {
    ...
    private boolean noEnchantCap$staticRepairCost(ItemStack instance, int repairCost) {
        return !NoEnchantCap.getConfig().isSTATIC_REPAIR_COST();//!this.player.level.getGameRules().getBoolean(NoEnchantCap.STATIC_REPAIR_COST);
    }
    ...
    private boolean noEnchantCap$fairAnvilCost(Player instance, int levels) {
        if (NoEnchantCap.getConfig().isFAIR_EXPERIENCE_COST()){//this.player.level.getGameRules().getBoolean(NoEnchantCap.FAIR_EXPERIENCE_COST)) {
